...
Use the `OpenSSF Scorecard` template to add the standard repository-local
analysis to a new public repository. Existing repositories keep their own
publishing workflow because Scorecard's result service restricts the job's
actions and OIDC context; the template keeps those requirements visible and
reviewable in the consuming repository.
